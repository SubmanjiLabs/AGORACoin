...
CzAGORTracker::CzAGORTracker(std::string strWalletFile)
{
...
CzAGORTracker::~CzAGORTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzAGORTracker::Init()
{
...
bool CzAGORTracker::Archive(CMintMeta& meta)
{
...
bool CzAGORTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzAGORTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzAGORTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzAGORTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzAGORTracker::GetSerialHashes()
{
...
CAmount CzAGORTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzAGORTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzAGORTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzAGORTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzAGORTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzAGORTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzAGORTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzAGORTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzAGORTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzAGORTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzAGORTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzAGORTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzAGORTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzAGORTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzAGORTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzAGORTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzAGORTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzagor from DB\n", __func__, listDeterministicDB.size());
    }
