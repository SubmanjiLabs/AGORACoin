...
#ifndef AGORA_ZAGORWALLET_H
#define AGORA_ZAGORWALLET_H
...
public:
    CzAGORWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZAGOR(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZAGOR(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //AGORA_ZAGORWALLET_H
