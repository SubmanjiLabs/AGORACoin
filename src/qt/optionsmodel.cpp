...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::AGOR);
    ...
    if (!settings.contains("nAnonymizeAGORAAmount"))
        settings.setValue("nAnonymizeAGORAAmount", 1000);

    nAnonymizeAGORAAmount = settings.value("nAnonymizeAGORAAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeAGORAAmount"))
        SoftSetArg("-anonymizeagoraamount", settings.value("nAnonymizeAGORAAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in agora.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAGORAAmount:
            return QVariant(nAnonymizeAGORAAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAGORAAmount:
            nAnonymizeAGORAAmount = value.toInt();
            settings.setValue("nAnonymizeAGORAAmount", nAnonymizeAGORAAmount);
            emit anonymizeAGORAAmountChanged(nAnonymizeAGORAAmount);
            break;
        case CoinControlFeatures:
