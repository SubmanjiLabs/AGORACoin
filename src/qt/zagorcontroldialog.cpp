...
#include "ui_zagorcontroldialog.h"
...
std::set<std::string> ZAgorControlDialog::setSelectedMints;
std::set<CMintMeta> ZAgorControlDialog::setMints;

ZAgorControlDialog::ZAgorControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZAgorControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZAgorControlDialog::~ZAgorControlDialog()
{
    delete ui;
}

void ZAgorControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZAgorControlDialog::updateList()
{
...
void ZAgorControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZAgorControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZAgor_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZAgorControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZAgorControlDialog::GetSelectedMints()
{
...
void ZAgorControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
