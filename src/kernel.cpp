...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZAgorStake(spend));
    } else {
        ...
        CAgorStake* agorInput = new CAgorStake();
        agorInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(agorInput);
    }
