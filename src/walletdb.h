...
public:
    ...
    bool WriteZAGORSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZAGORSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZAGORSeed_deprecated(uint256& seed);
    bool EraseZAGORSeed();
    bool EraseZAGORSeed_deprecated();

    bool WriteZAGORCount(const uint32_t& nCount);
    bool ReadZAGORCount(uint32_t& nCount);
