...
CzAGORWallet::CzAGORWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zagor seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZAGORSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZAGORSeed_deprecated()) {
                    LogPrintf("%s: Updated zAGOR seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zagor seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zagor wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzAGORWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZAGORCount(nCountLastUsed);
    else if (!walletdb.ReadZAGORCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzAGORWallet::Lock()
{
    seedMaster = 0;
}

void CzAGORWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzAGORWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZAGOR(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzAGORWallet::LoadMintPoolFromDB()
{
...
void CzAGORWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzAGORWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzAGORWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zagorTracker->HasPubcoinHash(pMint.first)) {
...
bool CzAGORWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZAGOR(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zagorTracker which also adds to database
    pwalletMain->zagorTracker->Add(dMint, true);
...
void CzAGORWallet::SeedToZAGOR(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzAGORWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzAGORWallet::GenerateDeterministicZAGOR(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzAGORWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzAGORWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
